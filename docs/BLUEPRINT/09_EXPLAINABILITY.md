...
## 12.1. Tại sao cần Explainability?
...
## 12.2. Giải pháp: SHAP (SHapley Additive exPlanations)

Sử dụng **`shap.TreeExplainer`** — được tối ưu cho **tất cả tree-based models** (XGBoost, LightGBM, Random Forest), **rất nhanh** (~1-5ms per prediction):
