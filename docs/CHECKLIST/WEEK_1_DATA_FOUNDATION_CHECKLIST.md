...
- Thiết lập `data/train_raw/`, `data/simulation_raw/`, `data/retrain_raw/`, `data/bronze/`, `data/silver/`
...
- [ ] Có cấu trúc thư mục dữ liệu rõ ràng cho train raw, simulation raw, retrain raw, bronze, silver, gold
...
- [ ] Bảo đảm `data/train_raw/` được dùng làm input baseline training, không đọc trực tiếp từ `dataset/` trong pipeline chính
...
- [ ] Thêm path config: `train_raw_data_path`, `simulation_raw_data_path`, `retrain_raw_data_dir`, `bronze_data_path`, `silver_data_path`, `gold_data_dir`
...
- [ ] Chốt một cách chuẩn để populate `data/train_raw/` từ `dataset/2019-Oct.csv.gz`
- [ ] Chốt `data/simulation_raw/2019-Nov.csv.gz` là source cho Online Simulation
- [ ] Chốt retraining dùng DB export vào `data/retrain_raw/<window_id>/`, không train trực tiếp từ simulation raw
...
- [ ] Có quy ước nhập dữ liệu vào train/simulation/retrain raw rõ ràng và nhất quán
...
- [ ] Giữ `price = null`, loại bỏ `price <= 0`
- [ ] Loại duplicate theo canonical event key
...
- [ ] Xác nhận `2019-Oct.csv.gz` chỉ được hard-code như baseline training contract
...
- [ ] Không có stage nào đọc trực tiếp `data/simulation_raw/2019-Nov.csv.gz` để train baseline
...
- [ ] `price = null` được giữ lại, `price <= 0` không đi vào silver
- [ ] Duplicate theo canonical key không đi vào silver
...
- [ ] Bước 3: chốt raw intake rule cho `data/train_raw/`, `data/simulation_raw/`, `data/retrain_raw/`
